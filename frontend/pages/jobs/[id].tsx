...
const JobPage = () => {
	...
	const [applyJob, { loading: applyLoading, error: applyError }] = useMutation(APPLY_JOB, {
	...
	const handleApply = useCallback(async () => {
		confirm('Are you sure you want to apply to this job?') && (await applyJob())
		router.push('/jobs')
	}, [applyJob, router])
		...
		<Dashboard>
			<div className="flex h-[80vh] w-full items-center justify-center rounded-xl">
				<div className="h-full w-full space-y-6 group p-4 rounded-3xl bg-white border border-gray-200/50  dark:shadow-none dark:border-gray-700 dark:bg-gray-800 bg-opacity-50 shadow-2xl shadow-gray-600/10 hover:border-primary dark:hover:border-primaryDark">
					...
					<h5 className="text-xl text-gray-600 dark:text-gray-300">{job.company}</h5>
					<span className="block text-gray-500 dark:text-gray-400">
						{job.location} - {job.jobType}
					</span>

					<span className="block text-gray-500 dark:text-gray-400">Rate per hour: {job.ratePerHour} U$D</span>
					<DetailArrayField label="Tools" array={job.tools} />
					<DetailArrayField label="Disciplines" array={job.disciplines} />
					<span className="block text-gray-500 dark:text-gray-400">{job.jobDescription}</span>
				</div>
			</div>
		</Dashboard>
	)
